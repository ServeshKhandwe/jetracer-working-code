...
This project implements a new navigation system for the Jetracer robot that uses OpenAI models to make movement decisions based on motion capture data. Unlike the existing SafeLLMRA system, this controller operates without safety constraints and focuses on comparing different OpenAI model performances.
...
├── launch/
│   └── openai_mocap_controller.launch  # ROS launch file
└── scripts/
    ├── openai_mocap_controller.py      # Main controller node
...
The equivalent command to the existing system is:
```bash
roslaunch openai_mocap_controller openai_mocap_controller.launch
```
